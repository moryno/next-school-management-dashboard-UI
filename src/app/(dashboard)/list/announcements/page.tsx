...
const AnnouncementListPage = () => {
    ...
    const renderRow = useCallback(( record: AnnouncementType) => (
  ...
  return (
    <section className="bg-white p-4 rounded-md shadow-md flex-1 m-4 mt-0">
        <article className="flex items-center justify-between">
            <h1 className="hidden md:block text-sm font-semibold">Announcements</h1>
         ...
         <GridComponent columns={columns} renderRow={renderRow} datasource={announcementsData} />
         <Pagination />
    </section>
  )
}
